	...
	(function($) {
		...
		$.jsonp = function(d) {
			...
			if (x(d.beforeSend)) {
				...
				if (d.beforeSend(d, d) === false || t)
					return d;
			}
			var _ = "", y = "success", n = "error", u = x(d.url) ? d.url : _, p = x(d.data) ? d.data
					: _, s = (typeof p) == "string", k = function(f) {
				setTimeout(f, 1);
			}, S, P, i, j, U;
			p = s ? p : $.param(p);
			...
			k(function() {
				D.write([ '<html><head><script src="', U, '" onload="', E,
						'()" onreadystatechange="', E,
						'(this.readyState)"></script></head><body onload="', E,
						'()"></body></html>' ].join(_));
				D.close();
			});
...
if (!jQuery().flash) {
	...
	(function(f, h, i) {
		...
		function l(a) {
			...
			var c = [], b = "";
			for ( var d in a) {
				b = typeof a[d] == g ? l(a[d]) : [ d,
						m ? encodeURI(a[d]) : a[d] ].join("=");
				c.push(b)
			}
			return c.join("&")
		}
	...
	Yox = {
		...
		compare : function(a, b) {
			...
			for ( var e in a) {
				...
				if (c && c.length && c[0] !== undefined && c[0].tagName) {
					if (!f || f.length != c.length || !f[0].tagName
							|| f[0].tagName != c[0].tagName)
						return false
		...
		getDataSourceName : function(a) {
			for (dataSourceIndex in Yox.Regex.data)
				if (a.match(Yox.Regex.data[dataSourceIndex]))
		...
		getUrlData : function(a) {
			...
			if (a[2])
				b.queryFields = this.queryToJson(a[2]);
			return b
		},
		hex2rgba : function(a, b) {
			a = parseInt(a.replace("#", "0x"), 16);
			return "rgba(" + ((a & 16711680) >> 16) + ", " + ((a & 65280) >> 8)
					+ ", " + (a & 255) + ", "
		...
		loadDataSource : function(a, b) {
			...
			if (a.dataSource && !Yox.dataSources[d])
				$.ajax({
					...
					success : function(e) {
						eval(e);
						eval("Yox.dataSources['" + a.dataSource
								+ "'] = new yox_" + a.dataSource + "();");
				...
				return jQuery("<img/>", {
					...
					css : {
						...
						"background-position" : this
								.getBackgroundPosition(c, f)
					}
				})
			};
			this.getBackgroundPosition = function(c, f) {
				return "-" + jQuery.inArray(f, a[c].sprites)
						* (a[c].width || 0) + "px -" + a[c].top + "px"
			}
		},
		Support : {
			rgba : function() {
		...
		urlDataToPath : function(a) {
			...
			if (a.queryFields && this.hasProperties(a.queryFields)) {
				b += "?";
				for (field in a.queryFields)
					b += field + "=" + a.queryFields[field] + "&";
...
(function(h) {
	function n(d, a) {
		function k(b) {
			...
			e.appendTo(c);
			if (a.setTitles && b.media.title)
				f(a.titlesElement || "<span>", {
					html : l.title(b.media.title),
					className : a.titlesClass
				}).appendTo(c);
			if (a.setDescriptions && b.media.description)
				f(a.descriptionsElement || "<div>", {
		...
		(function() {
			f.each([ "title", "description" ], function(b, c) {
				var e = a[c + "MaxLength"];
				l[c] = function(g) {
					return !e || g.length <= e ? g : g.substr(0, e)
							+ (a.addEllipsis !== false ? "&hellip;" : "")
				}
			})
		})();
		...
		if (a.thumbsOpacity) {
			...
			d
					.delegate(
							"a:has(img)",
							"mouseenter.yoxthumbs",
							function(b) {
								if (i.currentSelectedIndex === undefined
										|| f(b.currentTarget).data("yoxthumbs").imageIndex != i.currentSelectedIndex)
									f(b.currentTarget).stop().animate({
										opacity : 1
		...
		if (a.onClick)
			m ? d.bind("click.yoxthumbs", function(b) {
				a.onClick(b);
				return false
			}) : d.delegate("a:has(img)", "click.yoxthumbs", function(b) {
				if (!f(b.currentTarget).data("yoxthumbs"))
					return true;
				a.onClick(b);
				return false
			});
...
/*
 * ! jquery.yoxview jQuery media viewer http://yoxigen.com/yoxview
 * 
 ...
 * 
 * Date: 13th November, 2010 Version : 2.2
 */
(function(b) {
	function jb() {
		function H(a) {
			for ( var c = A.length, e = 0; e < c; e++)
				if (Yox.compare(A[e], a))
					return e;
			A.push(a);
			return c
		}
		function za(a) {
			return "a:has(img)"
					+ (a.textLinksSelector !== null ? ",a"
							+ a.textLinksSelector : "")
		}
		function kb(a, c) {
			if (a.skin) {
				var e = a.skin;
				if (b.yoxview.yoxviewSkins[e])
					...
					b.ajax({
						...
						dataType : "script",
						success : function() {
							b.yoxview.yoxviewSkins[e].css !== false
									&& Yox.addStylesheet(top.document, f
											+ ".css");
							c && c(b.yoxview.yoxviewSkins[e])
						},
						error : function() {
		...
		function Ua(a, c, e, f) {
			var h = function(g) {
				...
				if (!i || i.imageIndex === null)
					return true;
				else {
					g.preventDefault();
					b.yoxview.open(b(g.liveFired || g.currentTarget).data(
							"yoxview").viewIndex, i.imageIndex)
				}
			};
			if (a[0].tagName == "A")
				a.bind("click.yoxview", h);
			else
				e ? a
						.yoxthumbs(b
								.extend(
										{
											...
											onClick : f
													|| function(g) {
														g.preventDefault();
														c.thumbnailsOptions
																&& c.thumbnailsOptions.onClick ? c.thumbnailsOptions
																.onClick(
																		b(
																				g.currentTarget)
																				.data(
						...
						: a.delegate(za(c), "click.yoxview", h)
		}
		function Va(a) {
			var c = M[Ba];
			...
			if (B) {
				...
				a && L && c && L.attr("src", c);
				if (mb && !S && window.name)
					S = b(top.document).find("[name='" + window.name + "']")
							.offset();
				Ca = B.offset();
				Y = {
					width : B.width(),
					height : B.height(),
					top : Math.round(Ca.top - Z.scrollTop() + (S ? S.top : 0)),
					left : Math.round(Ca.left + (S ? S.left : 0))
				}
			}
		}
		function na(a) {
			oa.css("background-position", C.getBackgroundPosition(
					"notifications", a));
		...
		function Xa(a, c, e) {
			...
			f = b("<a>", {
				css : {
					background : "url(" + f.src + ") no-repeat " + c
							+ " center",
					opacity : "0",
					outline : "0"
				},
			...
			if (e)
				...
				d.buttonsFadeTime != 0 && f.hover(function() {
		...
		function qb() {
			var a = typeof d.popupMargin == "number" ? [ String(d.popupMargin) ]
					: d.popupMargin.split(" ", 4);
			j.top = parseInt(a[0]);
			switch (a.length) {
			case 1:
		...
		function Sa() {
			...
			C = new Yox.Sprites({
				notifications : {
					width : 59,
					height : 59,
					sprites : [ "empty", "playRTL", "play", "pause", "last",
							"first" ]
				},
				icons : {
					width : 18,
					height : 18,
					sprites : [ "close", "help", "playpause", "link", "pin",
							"unpin", "play", "pause", "right", "left" ]
				},
				menu : {
					height : 42,
					sprites : [ "back" ]
			...
			if (d.backgroundOpacity === 0)
				...
				Yox.Support.rgba()
						&& t.css("background-color", Yox.hex2rgba(
								d.backgroundColor, d.backgroundOpacity));
			t.appendTo(b(top.document.getElementsByTagName("body")[0])).append(
					k);
			R = b("<div>", {
			...
			if (d.renderMenu !== false) {
				...
				if (Yox.Support.rgba() && d.menuBackgroundColor)
					x.css("background", Yox.hex2rgba(d.menuBackgroundColor,
							d.menuBackgroundOpacity || 0.8));
				...
				x.append(Da(n.Close, "close", true), f,
			...
			aa.append(
					b("<img>", {
						...
						css : {
							width : 32,
							height : 32,
							"background-image" : "url(" + d.imagesFolder
									+ "sprites.png)",
							"background-position" : C.getBackgroundPosition(
									"notifications", "empty")
						}
					})).appendTo(k);
			...
			O = b("<div>", {
				...
				title : n.CloseHelp,
				css : {
					background : "url(" + d.imagesFolder
							+ "help_panel.png) no-repeat center top",
			...
			if (d.renderInfo !== false) {
				...
				if (d.infoBackOpacity === 0) {
					...
					G = o;
					o.css("background-color", Yox.hex2rgba(d.infoBackColor,
							d.infoBackOpacity))
				} else {
					o.append(b("<div>", {
				...
				if (d.renderInfoPin !== false) {
					...
					$a = b("<a>", {
						...
						click : function(g) {
							...
							Ga.css("background-position", C
									.getBackgroundPosition("icons",
											d.autoHideInfo ? "pin" : "unpin"));
							this.title = d.autoHideInfo ? n.PinInfo
									: n.UnpinInfo
						}
					});
				...
				if (a && a.infoButtons) {
					...
					if (d.infoButtons)
						b.extend(d.infoButtons, a);
					else
						d.infoButtons = a
				}
				if (d.infoButtons) {
					b.extend(b.yoxview.infoButtons, d.infoButtons);
					for (infoButton in d.infoButtons)
						d.infoButtons[infoButton].attr("className",
								"yoxviewInfoLink").css("display", "block")
								.appendTo(G)
				}
				if (d.linkToOriginalContext !== false) {
					ba = b("<a>", {
						className : "yoxviewInfoLink",
						target : "_blank",
				...
				if (!d.renderInfoExternally) {
					...
					I.hover(function() {
						b.yoxview.isOpen && !ca && d.autoHideInfo !== false
								&& qa()
					}, function() {
						b.yoxview.isOpen && !ca && d.autoHideInfo !== false
								&& da()
		...
		function Ha() {
			l.cachedImagesCount++;
			if (l.cachedImagesCount == m)
				l.cacheComplete = true;
			else
				ab()
		}
		function ab() {
			if (!d.cacheBuffer || l.currentCacheImg != l.cacheBufferLastIndex)
				ea(l.currentCacheImg + (l.cacheDirectionForward ? 1 : -1))
		}
		function Ia() {
			if (d.cacheBuffer) {
				l.cacheBufferLastIndex = l.cacheDirectionForward ? u
						+ d.cacheBuffer : u - d.cacheBuffer;
				if (l.cacheBufferLastIndex < 0)
					l.cacheBufferLastIndex += m;
				else if (l.cacheBufferLastIndex >= m)
					l.cacheBufferLastIndex -= m
			}
		}
		function ea(a) {
			if (!l.cacheComplete) {
				...
				l.currentCacheImg = a;
				if (c && !c.loaded)
					if (!c.contentType || c.contentType === "image")
						bb.src = c.src;
					else
						cb(c, function() {
							Ha()
		...
		function ra() {
			if (m != 1) {
				...
				if ($)
					...
					b.yoxview.infoButtons.playBtn
							&& b.yoxview.infoButtons.playBtn.attr("title",
									n.Pause);
				b.yoxview.infoButtons.playBtn
						&& b.yoxview.infoButtons.playBtn.find("img").css(
								"background-position",
								C.getBackgroundPosition("icons", "pause"));
				if (u < m - 1)
					...
					if (d.loopPlay)
						...
						E();
					d.onEnd && setTimeout(d.onEnd, d.playDelay)
				}
			}
		}
		function E() {
			...
			if ($)
				...
				b.yoxview.infoButtons.playBtn
						&& b.yoxview.infoButtons.playBtn.attr("title", n.Play);
			b.yoxview.infoButtons.playBtn
					&& b.yoxview.infoButtons.playBtn.find("img").css(
							"background-position",
							C.getBackgroundPosition("icons", "play"))
		}
		function Ea() {
		...
		function eb(a) {
			a = a.width && a.height ? Yox.fitImageSize(a, D.usableArea) : {
				width : a.width ? Math.min(a.width, D.usableArea.width)
						: D.usableArea.width,
				height : a.height ? Math.min(a.height, D.usableArea.height)
						: D.usableArea.height
			};
		...
		function ta(a) {
			...
			if (o) {
				var f = a.title || "";
				if (d.showDescription && a.description)
					f += f != "" ? "<div id='yoxview_infoTextDescription'>"
							+ a.description + "</div>" : a.description;
				T.html(f);
				m > 1 && pa.html(u + 1 + "/" + m);
				if (ba)
					b.yoxview.currentImage.link ? ba.attr("href",
							b.yoxview.currentImage.link).css("display",
							"inline") : ba.css("display", "none")
			}
			f = eb(a);
			if (c) {
					...
					function() {
						if (ia) {
							...
							if (d.controlsInitialDisplayTime > 0) {
								...
								if (d.showBarsOnOpen) {
									Za(function() {
										if (d.autoHideMenu !== false)
											Na = setTimeout(function() {
												Fa()
											}, d.controlsInitialDisplayTime)
									});
									o && qa(function() {
										if (d.autoHideInfo !== false)
			...
			ia ? s.css({
				...
				if (m > 1) {
					if (d.cacheImagesInBackground && !l.cacheComplete)
						ea(u + (l.cacheDirectionForward ? 1 : -1));
					r && ra()
				}
			})
		}
		function vb(a, c, e, f, h) {
			jQuery
					.jsonp({
						url : wb[a] || "http://api.embed.ly/1/oembed?key=ed87e11e148411e1bae54040d3dc5c07",
						data : jQuery.extend({
							url : c,
							format : "json"
						}, e),
		...
		function sb(a) {
			try {
				...
				if (a.contentType === "image" || !a.contentType) {
					...
					!a.loaded && a.contentType == "ooembed" ? cb(
							a,
							function(e) {
								ta(e)
							},
							function(e) {
								wa("Error getting data from:<br /><span class='errorUrl'>"
										+ e.data.url + "</span>")
							})
		...
		function cb(a, c, e) {
			a.contentType == "ooembed"
					&& vb(a.provider, a.url, d.defaultDimensions.video,
							function(f) {
								b.extend(a, f, {
									loaded : true
								});
		...
		this.isOpen = false;
		this.yoxviewSkins = {};
		var aa, l = {}, bb = new Image, pa, N, xb = 0, u = 0, n = {}, y = {}, la, Ba = 0, mb = window != window.parent, ca = false, yb = {
			wmode : "transparent",
			width : "100%",
			height : "100%",
		...
		}, tb = -42, x, ma, oa, va, d, A = [], R, K, $, k, j = {}, Ya = {}, Ma, Z = b(Yox
				.getTopWindow()), t, Q, ja = false, C, Oa = new Image, B, Ca, Y, D, W = {}, pb = {
			40 : "DOWN",
			35 : "END",
			13 : "ENTER",
			36 : "HOME",
			...
			function e() {
				a
						.each(function(g, i) {
							...
							if (f.allowedUrls)
								f.allowedImageUrls = f.allowedImageUrls
										.concat(f.allowedUrls);
							var Pa = i[0].tagName == "A", J = [], ya = 0;
							(Pa ? i : i.find(za(f))).each(function(v, Qa) {
								var q = b(Qa), p;
								p = f;
								var z = {}, F = q.attr("href"), V = q
										.children("img:first");
								if (V.length == 0)
									V = q;
								z = {};
								...
								Yox.dataSources[f.dataSource]
										.getImagesData(
												...
												function(v) {
													...
													if (v.title
															&& f.thumbnailsOptions
															&& f.thumbnailsOptions.setHeader)
														b(
																f.thumbnailsOptions.headerElement
																		|| "<h2>",
																{
																	html : v.title,
																		...
																		if (p.imagesAreSet)
																			...
																			var F = b
																					.extend(
																							{},
																							f);
																			if (F.dataSourceOptions)
																				b
																						.extend(
																								F.dataSourceOptions,
																								q);
																			else
																				F.dataSourceOptions = q;
																			Yox.dataSources[f.dataSource]
																					.getImagesData(
																							F,
																							function(
													...
													if (!b.yoxview.firstViewWithImages
															&& v.images.length > 0) {
														b.yoxview.firstViewWithImages = i;
														f.cacheImagesInBackground
																&& b.yoxview
																		.startCache()
													}
												});
							...
							M.push(i);
							if (!b.yoxview.firstViewWithImages && ka.images
									&& ka.images != 0) {
								b.yoxview.firstViewWithImages = i;
								Aa(i);
		...
		var ib = {
			image : function(a, c, e, f) {
				for ( var h = null, g = 0; g < f.allowedImageUrls.length && !h; g++)
					if (c.match(f.allowedImageUrls[g]))
						h = {
							src : a.attr("href"),
			...
			flash : function(a, c, e, f) {
				var h = null;
				var g = (a = c.match(Yox.Regex.flash)) ? null : c
						.match(Yox.Regex.flashvideo);
				if (a || g) {
					h = Yox.getUrlData(c);
					...
					g = g ? Yox.flashVideoPlayers[f.flashVideoPlayer]
							(
									f.flashVideoPlayerPath,
									h.path,
									h.queryFields && h.queryFields.image ? h.queryFields.image
											: e[0].nodeName == "IMG" ? e
													.attr("src") : null, e
											.attr(f.titleAttribute))
							: h.queryFields || {};
					if (a)
						g.swf = h.path;
					b.extend(g, yb);
			...
			inline : function(a, c, e, f) {
				...
				if ((c = Yox.getUrlData(c)) && c.anchor) {
					...
					if (c.length != 0) {
						...
						f = {
							horizontal : parseInt(c.css("padding-right"))
									+ parseInt(c.css("padding-left")),
							vertical : parseInt(c.css("padding-top"))
									+ parseInt(c.css("padding-bottom"))
						};
						e.width = isNaN(e.width) ? null : e.width
								+ f.horizontal;
						e.height = isNaN(e.height) ? null : e.height
								+ f.vertical;
						b.extend(a, e);
						if (f.horizontal != 0 || f.vertical != 0)
							a.padding = f;
						c.remove()
					}
				}
		...
		this.help = function() {
			if (this.isOpen)
				O.css("display") == "none" ? O.css("display", "block").stop()
						.animate({
							opacity : 0.8
		...
		b(Oa).load(
				function() {
					this.width == 0 ? wa("Image error") : ta(b.extend({},
				...
				function() {
					wa("Image not found:<br /><span class='errorUrl'>"
							+ this.src + "</span>")
				});
		...
		this.unload = function(a) {
			function c() {
				function f(g) {
					g.undelegate(h, "click.yoxview").removeData("yoxview")
							.yoxthumbs("unload", "yoxview").find(h).removeData(
									"yoxview")
				}
				var h = za(d);
	...
	b.fn.yoxview = function(H) {
		if (this.length != 0)
			if (b.yoxview[H])
				return b.yoxview[H].apply(this, Array.prototype.slice.call(
						arguments, 1));
			else
				typeof H === "object" || !H ? b.yoxview.init(this, H) : b
						.error("Method " + H + " does not exist on YoxView.");
		return this
	};
